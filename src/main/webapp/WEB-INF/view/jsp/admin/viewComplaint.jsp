				  ...
				  <table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
						<%@taglib uri="http://java.sun.com/jstl/core_rt"
							...
							<tr>
								<td>${j.count}</td>
								<%-- <td>${complaint.registerVO.name }</td> --%>
								<td>${complaint.complaintSubject}</td>
								<td>${complaint.complaintDescription}</td>
								<td><a href="${complaint.complaintFileLink}"><i class="fa fa-paperclip" aria-hidden="true"></i>View</a></td>
								<%-- <td>${complaint.complaintStatus }</td> --%>
								<td>
								<c:choose>
									   <c:when test="${complaint.complaintStatus =='Pending'}">
									   		<a href="reply?id=${complaint.id }"> <div class="label label-table label-warning">Pending   (Reply)</div></a>
									   </c:when> 
									   
									   <c:otherwise>
									   		<%-- <a href="reply?id=${complaint.id }" onclick="return false;">Reply</a> --%>
									   		<a href="javascript:void(0)" style="cursor: default;"><div class="label label-table label-info">Resolved</div></a>
									   	</c:otherwise>   
								</c:choose>
								</td>
								...
								<c:choose>
									   <c:when test="${complaint.complaintStatus =='Pending'}">
									   		<a href="reply?id=${complaint.id }">Reply</a>
									   </c:when> 
									   
									   <c:otherwise>
									   		<a href="reply?id=${complaint.id }" onclick="return false;">Reply</a>
									   		<a href="javascript:void(0)" style="cursor: default;">Responded</a>
									   	</c:otherwise>   
								</c:choose>
								</td> --%>
								<td>${complaint.complaintDate}</td>
								<td>${complaint.replyDate}</td>
								<td><a href="deleteComplaint?id=${complaint.id}"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>
